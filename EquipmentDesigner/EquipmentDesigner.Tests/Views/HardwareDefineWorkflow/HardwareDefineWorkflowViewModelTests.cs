...
namespace EquipmentDesigner.Tests.Views.HardwareDefineWorkflow
{
    public class HardwareDefineWorkflowViewModelTests
    {
        ...
        public async Task CompleteWorkflowAsync_WhenEquipmentStartType_SavesWorkflowWithTreeNodes()
        {
            ...
            var sessions = await workflowRepository.LoadAsync();

            var savedWorkflow = sessions.FirstOrDefault(w => w.Id == viewModel.HardwareId);
            savedWorkflow.Should().NotBeNull();
            savedWorkflow.Children.Should().NotBeEmpty();
        ...
        public async Task CompleteWorkflowAsync_UpdatesExistingWorkflow()
        {
            ...
            sessions.Count(w => w.Id == viewModel.HardwareId).Should().Be(1);
        }
        ...
        public async Task UploadToServer_MapsTreeDataToFlatHardwareLists()
        {
            ...
            var uploadedSessions = await uploadedRepo.LoadAsync();
            
            uploadedSessions.Should().HaveCount(1);
            uploadedSessions[0].HardwareType.Should().Be(HardwareType.Equipment);
            uploadedSessions[0].Children.Should().NotBeNullOrEmpty();
        }
        ...
        public async Task UploadToServer_SetsComponentStateToUploaded()
        {
            ...
            uploadedSessions.Should().HaveCount(1);
            uploadedSessions[0].State.Should().Be(ComponentState.Uploaded);
        }
        ...
        public async Task UploadToServer_PreservesParentChildRelationshipsInUploadedData()
        {
            ...
            var session = uploadedSessions[0];
        ...
        public async Task UploadToServer_UpdatesBothWorkflowsAndUploadedHardwaresFiles()
        {
            ...
            sessions.Add(new HardwareDefinition
            {
                Id = viewModel.HardwareId,
            ...
            sessions.Any(s => s.Id == viewModel.HardwareId).Should().BeFalse();
        }
        ...
        public async Task SaveWorkflowStateAsync_UsesWorkflowRepositoryInsteadOfOldRepository()
        {
            ...
            sessions.First().Id.Should().Be(viewModel.HardwareId);
        }
        ...
        public async Task SaveWorkflowStateAsync_PreservesEntireTreeStructureInTreeNodesProperty()
        {
            ...
            var session = sessions.FirstOrDefault(s => s.Id == viewModel.HardwareId);
            session.Should().NotBeNull();
            session.Children.Should().NotBeEmpty();
        ...
        public async Task SaveWorkflowStateAsync_UpdatesExistingWorkflowSessionByWorkflowId()
        {
            ...
            sessions.Count(s => s.Id == viewModel.HardwareId).Should().Be(1);
            
            var session = sessions.First(s => s.Id == viewModel.HardwareId);
            session.Name.Should().Be("UpdatedName");
        }
